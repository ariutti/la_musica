 ...
 * * 
 * 2018-10-16 modiichje in cantiere per fare si che VVVV possa mantenere 
 * acceso in giallo l'ultimo tasto selezionato. 
 * 
 * Di base ora Arduino non si occupa più di triggerare attacchi o rilasci
 * E' VVVV che, inviando una stringa di 10 valori numerici
 * si occupa di fare questo.
 * 
 * I possibili valori contenuti in questa stringa son 3:
 * 0: il tasto deve restare spento;
 * 1: il tasto deve colorarsi di blu;
 * 2: il tasto deve colorarsi di giallo.
 */
...
void button_update( int i )
{
  ...
  if( !btnState )
  {
    ...
    //2018-10-16 modification to the color of the selected key
    //setColor(i, 0xFF, 0xFF, 0x00); // YELLOW
  ...
  {
    //2018-10-16 modification to the color of the selected key
    //setColor(i,0x00, 0x00, 0xFF); // BLUE
...
void printAllKeys()
{
  // 2018-08-09: REVERSE THE ORDER OF THE BITS
  for(int i=0; i<NKEYS; i++)
  {
    Serial.print( keys[9-i].lastStateBtn );
  }  
  Serial.println(";");
}
...
void getSerialData()
{
  while (Serial.available() > 0) 
  {
    ...
    if (c == ';')
    {
      ...
      {
        // 2018-08-09: REVERSE THE INCOMING MESSAGE HERE
